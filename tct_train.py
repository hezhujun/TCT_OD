...
def main(args):
    dataset = TCTDataset(args.image_root, "dataset/train.json", get_transforms(True))
    dataset_train = TCTDataset(args.image_root, "dataset/train.json", get_transforms(False))
    ...
    data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, collate_fn=utils.collate_fn)
    dataset_train = torch.utils.data.Subset(dataset_train, [i for i in range(50)])
    data_loader_train = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=4, collate_fn=utils.collate_fn)
    dataset_val = torch.utils.data.Subset(dataset_val, [i for i in range(50)])
    data_loader_val = DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, num_workers=4, collate_fn=utils.collate_fn)
    dataset_test = torch.utils.data.Subset(dataset_test, [i for i in range(50)])
    data_loader_test = DataLoader(dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=4, collate_fn=utils.collate_fn)
    ...
    for epoch in range(epoch, args.epochs):
        ...
        evaluate(model, data_loader_train, device, epoch, log_train, head="Train:")
